...
class ConstantInput(nn.Module):
    def __init__(self, channel, size1=4,size2=4):
        super().__init__()

        self.input = nn.Parameter(torch.randn(1, channel, size1, size2))
...
class ConstantInputPatch(nn.Module):
    def __init__(self, channel, size=4, device = "cuda"):
        super().__init__()

        self.input = nn.Parameter(torch.randn(1, channel, size, size))
        self.device = device

    def forward(self, input, h_start, w_start, crop_size = 64):
        batch = input.shape[0]
#         h_index = torch.cat([torch.arange(h,h+64).unsqueeze(0) for h in h_start], dim = 0).to(self.device)
#         w_index = torch.cat([torch.arange(w,w+64).unsqueeze(0) for w in w_start], dim = 0).to(self.device)
        out = self.input.repeat(batch, 1, 1, 1)
        idv = []
        for b in range(batch):
            idv.append(out[b,:, h_start[b]:h_start[b] + crop_size, w_start[b]:w_start[b] + crop_size].unsqueeze(0))
        out = torch.cat(idv, dim = 0)
        return out
...
class LFF(nn.Module):
    def __init__(self, hidden_size, coord_size = 3):
        super(LFF, self).__init__()
        self.ffm = ConLinear(coord_size, hidden_size, is_first=True)
...
class StyledConvNoNoise(nn.Module):
    def __init__(
        self,
        in_channel,
        out_channel,
