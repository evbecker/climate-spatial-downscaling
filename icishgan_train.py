...
def validate(network,dataloader,criterion,mode):
    ...
    for batch_num,data in enumerate(dataloader):
        ...
        fake_hr,_=network_g(coord,lr_img,prev_hr_img,noise)
...
def test(network,dataloader,mode):
    ...
    for batch_num,data in enumerate(dataloader):
        ...
        if mode!='ours':
            coord=coord[:,:3,:,:]
        noise=mixing_noise(1,style_dim,1,device)
        pred,_=network_g(coord,lr_img,prev_hr_img,noise)
...
#for l1_lambda in [0.2]:
        for mode in ['ours']:
            ...
            if mode=='ours':
                network_g=Generator(size1=dataset,size2=dataset,style_dim=style_dim,coord_size=4)
            else:
                network_g=Generator(size1=dataset,size2=dataset,style_dim=style_dim,coord_size=3)
            #network=AE(input_channels=1,num_layers=3,base_num=16)
            network_g=network_g.to(device)
            network_d=Discriminator(size1=dataset,size2=dataset,input_size=1,cond_size=512)
            ...
            for epoch in range(0,epochs):
                for batch,data in enumerate(train_loader):
                    ...
                    real_input_d=hr_img
                    real_pred=network_d(real_input_d,cond)
                ...
                with torch.no_grad():
                    ...
                    if total_loss<loss_max:
                        ...
                        if dataset==40:
                            if mode=='ours':
                                torch.save(network_g.state_dict(),'ic_generator'+str(l1_lambda)+'.pt')
                            else:
                                torch.save(network_g.state_dict(),'ic_generator'+str(l1_lambda)+'_.pt')
                        else:
                            if mode=='ours':
                                torch.save(network_g.state_dict(),'_ic_generator'+str(l1_lambda)+'.pt')
                            else:
                                torch.save(network_g.state_dict(),'_ic_generator'+str(l1_lambda)+'_.pt')
