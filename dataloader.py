...
class EraiCpcWrfDataset_General(Dataset):

    def __init__(self, img_path, mode='train', regions=['neus','nwus','seus','swus'], var_type='precip'):
        ...
        if mode!='new_test':
            for r in regions:
                if mode=='train':
                    ...
                    end_date='2002-01-01'
                while date!=end_date:
                    if os.path.exists(img_path+'/erai-'+r+f'-{var_type}-'+date+'.pt'):
                        self.erai.append('erai-'+r+f'-{var_type}-'+date+'.pt')
                    if os.path.exists(img_path+'/cpc-'+r+f'-{var_type}-'+date+'.pt'):
                        self.cpc.append('cpc-'+r+f'-{var_type}-'+date+'.pt')
                    if os.path.exists(img_path+'/wrf-'+r+'-precip-'+date+'.pt'):
                        self.wrf.append('wrf-'+r+'-precip-'+date+'.pt')
                    self.regions.append(r)
                    date=next_date(date)
                self.elevation[r]=torch.load(img_path+'/'+r+'-elevation-160x160.pt')
            ...
            end_date='3000-01-01'
            r='mwus'
            #print(os.path.exists(img_path+'/erai-'+r+'-{var_type}-'+date+'.pt'),os.path.exists(img_path+'/cpc-'+r+'-{var_type}-'+date+'.pt'),os.path.exists(img_path+'/wrf-'+r+'-{var_type}-'+date+'.pt'))
            while os.path.exists(img_path+'/erai-'+r+f'-{var_type}-'+date+'.pt') and os.path.exists(img_path+'/cpc-'+r+f'-{var_type}-'+date+'.pt') and os.path.exists(img_path+'/wrf-'+r+'-{var_type}-'+date+'.pt') and date!=end_date:
                self.erai.append('erai-'+r+f'-{var_type}-'+date+'.pt')
                self.cpc.append('cpc-'+r+f'-{var_type}-'+date+'.pt')
                self.wrf.append('wrf-'+r+f'-{var_type}-'+date+'.pt')
                self.regions.append(r)
                date=next_date(date)
            self.elevation[r]=torch.load(img_path+'/'+r+'-elevation-160x160.pt')
            self.latitude['mwus']=torch.linspace(35,45,160).unsqueeze(1)
    ...
    def __getitem__(self, index):
        ...
        # at the previous timestep
        slr_img = torch.load(os.path.join(self.img_path, self.erai[index])).unsqueeze(0)
        if self.var_type == 'precip':
            lr_img = torch.load(os.path.join(self.img_path, self.cpc[index])).unsqueeze(0)
        else:
            lr_img = torch.zeros((1,160,160))
        hr_img=torch.load(os.path.join(self.img_path, self.wrf[index])).unsqueeze(0)
        prev_index = (index - 1)%self.data_len
        prev_hr_img = torch.load(os.path.join(self.img_path, self.wrf[prev_index])).unsqueeze(0)
        coord=torch.zeros((4,slr_img.size(1),slr_img.size(2)))
        r=self.regions[index]
        coord[0,:,:]=date_converter(self.erai[index][-13:-3])
        coord[1,:,:]=self.latitude[r]
        coord[2,:,:]=self.longitude[r]
        coord[3,:,:]=self.elevation[r]
        #print(self.erai[index][-13:-3])
        return {'hr_img':hr_img.type(torch.FloatTensor),'lr_img':lr_img.type(torch.FloatTensor),
                'prev_hr_img':prev_hr_img.type(torch.FloatTensor),'coord':coord,'name':self.erai[index],
                'region':r,'slr_img':slr_img.type(torch.FloatTensor)}
