...
if __name__ == "__main__":
	...
	data_lr = xr.open_dataset('./ncdata/erai-jan2000-precip.nc')
	...
	nwin_precip_lr = crop_to_region(precip_lr, [38,48], [238,248])
	...
	nwin_precip_hr = crop_to_region(precip_hr, [38,48], [238,248])
	...
	lr_upscale = interpolate(precip_lr, lat_range, lon_range, steps=steps, method='nearest')
	plot_next(axarr[1,0], lr_upscale[10,:,:], lr_upscale.lat, lr_upscale.lon, min_max=[0,60], title='LR Interpolated Daily Precipitation')

	hr_upscale = interpolate(precip_hr, lat_range, lon_range, steps=steps, method='nearest')
